    ...
    with get_conn() as conn:
        ...
        cur.execute(
            f"""SELECT r.*,
                    COALESCE(
                        g.name,
                        INITCAP(REGEXP_REPLACE(REPLACE(REPLACE(REPLACE(
                            r.workflow,
                            '_', ' '), '__', ' '), '/', ' '), '.py', ''),
                            '([a-z])([A-Z])', E'\\1 \\2', 'g')))
                    ) AS flow_name
    ...
    with get_conn() as conn:
        ...
        cur.execute(f"""
            SELECT r.id, r.status,
                   ...
                   COALESCE(g.name, INITCAP(REGEXP_REPLACE(REPLACE(REPLACE(REPLACE(r.workflow, '_', ' '), '/', ' '), '.py', ''), '([a-z])([A-Z])', E'\\1 \\2', 'g')), 'unknown') AS flow_name,
...
def get_flow_analytics(days: int = 30, workspace_id: int | None = None) -> list:
    ...
    with get_conn() as conn:
        ...
        cur.execute("""
            SELECT
                r.graph_id,
                COALESCE(g.name, 'legacy: ' || INITCAP(REGEXP_REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(r.workflow, '_', ' '), '/', ' '), '.py', '')), '([a-z])([A-Z])', E'\\1 \\2', 'g')), 'unknown') AS flow_name,
                COUNT(*)                                               AS total,
                SUM(CASE WHEN r.status = 'succeeded' THEN 1 ELSE 0 END) AS succeeded,
                SUM(CASE WHEN r.status = 'failed'    THEN 1 ELSE 0 END) AS failed,
                ROUND(
