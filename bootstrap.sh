        ...
        {node.data.type === "action.sftp" && (
          <div style={{background:"#0f1117",border:"1px solid #2a2d3e",borderRadius:6,padding:"8px 10px",fontSize:10,color:"#94a3b8",marginTop:6,lineHeight:1.9}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:"0 10px",marginTop:6}}>
              <code style={{color:"#a78bfa"}}>list</code><span>→ {"{ files: [...], count }"}</span>
        ...
        {runStatus && (
          <div className="run-output-panel">
            <div className="run-output-title">
              ...
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                {runDurationMs !== undefined && (
                  <span style={{fontSize:10,color:"#64748b"}}>{runDurationMs}ms{runAttempts>1?` · ${runAttempts} attempts`:""}</span>
                )}
                <span className={`run-output-status ${statusCls}`}>{statusLabel}</span>
              </div>
            </div>
            {runInput !== undefined && (
              <>
                <div style={{fontSize:10,fontWeight:600,color:"#94a3b8",marginBottom:3,marginTop:4}}>▶ Input</div>
                <div className="run-output-body" style={{borderColor:"#1e3a5f",maxHeight:160}}>
                  {runInput?.__truncated
                    ? <span style={{color:"#64748b",fontStyle:"italic"}}>⚠ Input too large to display ({runInput.__size} chars)</span>
                    : (typeof runInput === "string" ? runInput : JSON.stringify(runInput, null, 2))
                  }
                </div>
              </>
            )}
            {runOutput !== undefined && runOutput !== null && (
              <>
                <div style={{fontSize:10,fontWeight:600,color: runStatus==="err"?"#f87171":"#4ade80",marginBottom:3,marginTop:6}}>
                  {runStatus==="err" ? "✗ Error" : runStatus==="skip" ? "↷ Skipped" : "✓ Output"}
                </div>
                <div className="run-output-body" style={{borderColor: runStatus==="err"?"#7f1d1d":"#14532d",maxHeight:200}}>
                  {runOutput?.__truncated
                    ? <span style={{color:"#64748b",fontStyle:"italic"}}>⚠ Output too large to display</span>
